...
> ⚠️ **核心数据保护原则**：灵感碎片是本系统最核心的资产，代码可重写，灵感不可丢失。部署前必须备份 `data/`，部署后必须用 `verify_deployment.py` 验证文件完整性。详见 `docs/PHILOSOPHY.md` 第九节（思想层）+ `docs/AI_WORKFLOW.md` 部署规则（行动层）。
...
### 4. 使用 deploy.sh 自动化部署脚本

版本升级时可使用 `deploy.sh` 固化确定性操作（备份/验证/迁移/重启），减少人工失误：

```bash
export PRODUCTION=/Users/xxx/project/hh-online   # 必须先设置
bash deploy.sh v0.4.1 v0.4.0
```

脚本按顺序执行 6 步，`set -e` 确保任何一步失败即退出。AI 仍负责场景判断、目录创建、代码部署、配置修改、隧道切换、文档更新。详见脚本顶部注释和 `docs/AI_WORKFLOW.md` 部署规则。

**边缘情况处理：**

| 场景 | 脚本行为 |
|------|---------|
| 首次部署（无旧版本） | 旧目录不存在 → 退出并提示，由 AI 先完成首次部署初始化 |
| 旧版本 Inbox 为空 | 无灵感文件匹配 → 输出警告跳过，不中断 |
| 新版本目录不存在 | 退出并提示，由 AI 先创建目录和部署代码 |
| plist 未创建 | 输出警告跳过服务重启，由 AI 手动处理 |
