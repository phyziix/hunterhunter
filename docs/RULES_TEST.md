...
| **文件变更星点同步** | 未实现 | 全局 | 灵感文件删除/修改时，星点不自动增减 |
...
## 五、星点获取方式与调整机制清单（2026-05-26）

### 5.1 星点获取方式汇总

| 序号 | 获取方式 | 代码位置 | 计算规则 | 单次上限 | 周期限制 |
|------|---------|---------|---------|---------|---------|
| 1 | **灵感采集** | `engine_capture.py:120` | 基础10 × 每日倍率(1/1.5/2/0.2) + 连续加成(7天+10%/30天+30%/90天+50%) | - | 每日不限次数 |
| 2 | **连接力奖励** | `engine_core.py:494` | ≥5→+50 / ≥20→+100 / ≥50→+200 | 每赛季各一次 | 赛季重置 |
| 3 | **内容发布** | `engine_core.py:906` | 750-1000随机 | 1000 | 每月5次 |
| 4 | **周回顾** | `engine_review.py:323` | 固定200 | 200 | 每周1次 |
| 5 | **月度战报** | `engine_review.py:421` | 固定500 | 500 | 每月1次 |

### 5.2 调整时星点增减机制

| 获取方式 | 删除/修改时星点增减 | 文件变更同步 | 重复提交防护 | 撤销机制 |
|---------|-------------------|-------------|-------------|---------|
| **灵感采集** | ❌ 无 | ❌ 不同步 | ✅ 有(相似度85%) | ❌ 无 |
| **连接力奖励** | ❌ 无 | - | ✅ 有(每赛季一次) | ❌ 无 |
| **内容发布** | ❌ 无 | - | ✅ 有(每月5次) | ❌ 无 |
| **周回顾** | ❌ 无 | - | ✅ 有(已修复) | ❌ 无 |
| **月度战报** | ❌ 无 | - | ✅ 有(已修复) | ❌ 无 |
...
| 风险类型 | 问题描述 | 影响 | 状态 |
|---------|---------|------|:----:|
| **文件变更不同步** | 删除灵感文件不会扣减星点 | 星点与实际灵感数量不一致 | ⏳ 未实现 |
| **无撤销机制** | 所有获取方式均无撤回功能 | 误操作无法挽回 | ⏳ 未实现 |
| **历史记录截断** | `star_income_history` 最多保留100条 | 旧记录丢失 | ⏳ 现状 |

### 5.4 记录方式

所有星点获取通过 `_log_star_income()` 记录到 `state["star_income_history"]`，包含：
- `source`: 获取来源（灵感采集/连接力奖励/内容发布/周回顾/月度战报）
- `amount`: 获得星点数
- `details`: 详细说明
- `timestamp`: 时间戳

### 5.5 数据一致性检查（2026-05-26）
...
| # | 问题 | 严重度 | 详情 |
|---|------|--------|------|
| 6 | **笔记数量严重不符** | 高 | state["total_notes"]=58，但实际只有14个灵感文件，差值44 |
| 7 | **缺少数据同步机制** | 中 | 灵感文件变更时不会自动同步 state |

### 5.7 影响分析

- **星点计算偏差**：历史测试数据导致星点统计不准确
- **连接力计算**：基于错误的笔记数量计算，可能影响奖励触发
- **每日倍率**：today_count 可能与实际采集次数不符

### 5.8 建议修复（综合评估后）

> 根据团队评估反馈调整：建议 2 降为 P2（v0.6+ 长期规划），建议 3 确认已通过部署脚本覆盖，无需额外操作。

| 序号 | 建议 | 优先级 | 状态 | 说明 |
|------|------|--------|------|------|
| 1 | 清理 state 中的测试数据 | **P0** | ✅ 已完成 | total_notes 从 58→14，与实际文件数一致 |
| 2 | 添加文件变更自动同步机制（方案A：启动时同步） | **P2** | ⏳ v0.6+ 长期规划 | 当前架构下场景覆盖有限，部署脚本已解决核心问题 |
| 3 | 添加定期数据一致性检查 | **无需额外操作** | ✅ 已覆盖 | deploy.sh Step 2/4 的 verify_deployment.py 已强制验证，灵感文件"只增不删"设计下日常检查预警价值低 |
