...
| v0.3 | 结构调整：采集+星点清理、前端精简、HTML/CSS/JS 拆分、engine.py 子模块拆分 | 📋 |
| v0.4 | 兑换模块开发（engine_exchange.py + feature flag，代码不删） | ❌ |
| v0.5 | 自定义挑战 | ❌ |
| v0.6 | 赛季主题系统 + app.js 按模块拆分 | ❌ |
| v0.7 | 能力值系统完整上线 | ❌ |
| v0.8 | 前端 Tab 导航 | ❌ |

**v0.2.5 修复详情**：
- 兑换模块下线（feature flag 关闭前端入口和路由，代码不动；开发并入 v0.4）
- 重复提交检测机制修正（目前去重逻辑不正确，仍可重复提交）
- 重复提交提示优化（检测到重复后弹出「建议情况」对话框）
- 标签提取过滤 `##`（正则匹配到 Markdown 标题格式）
- 相关灵感匹配规则打磨（IDF 加权 Jaccard 结果仍不合理）
