...
> 1. **判断场景**：生产环境是否已有当前版本运行？
>    - 是 → **增量更新**：只替换变更文件（跳步骤 3-5）
>    - 否 → **首次部署**：全流程
> 2. **备份数据**（首次部署必须；增量仅改前端则跳过）：`cp -a v0.X/data backups/before-deploy-$(date)/`
> 3. **停旧服**：`launchctl unload` 对应版本 plist
> 4. **创建目录 + 复制全量代码** + `python3 -m venv venv && pip install -r requirements.txt`
> 5. **数据迁移**：复制旧版数据 → 迁移 state.json 字段 → 替换 config.yaml（保留用户自定义字段）
> 6. **替换变更文件**（增量更新）：停服期间覆盖到生产目录
> 7. **启动 + 验证**：`launchctl load` → curl 版本号 + 关键 API + 前端关键字
> 8. **切换隧道**（如端口有变）：更新 tunnel plist/sh 指向新端口
> 9. **最终一步**：确认公网可访问，清理临时文件（migrate_state.py 等）
>
