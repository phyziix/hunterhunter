...
## 已完成：v0.4.2（标签宇宙 + 回顾按钮 + 数据修复 + Tunnel 自检）
...
| 任务 | 说明 | 状态 |
|------|------|:----:|
| **标签低频过滤** | app.js 过滤 count < 2 的标签 | ✅ |
| **周月排除** | 后端排除含周回顾/月回顾标签的文件 | ✅ |
| **初始化修复** | get_tag_cloud_data() 空时触发初始化 | ✅ |
| **文件同步** | 从 iCloud 复制生产环境灵感文件 | ✅ |
| **标签展开收起** | 点击文件名展开/收起内容 | ✅ |
...
| 任务 | 说明 | 状态 |
|------|------|:----:|
| **回顾按钮分离** | 提交/查看按钮分离居中，视觉区分 | ✅ |
| **周报月报始终显示** | 未提交时查看按钮也显示，点击提示 | ✅ |

### 数据一致性修复

| 任务 | 说明 | 状态 |
|------|------|:----:|
| **total_notes 修正** | state.json 中 total_notes 从 58→14 | ✅ |
| **建议优先级调整** | 建议 2 降为 P2（v0.6+ 长期规划）| ✅ |
| **建议 3 确认** | 部署脚本已覆盖，无需额外 cron 任务 | ✅ |

| 建议 | 原评估 | 调整后 |
|------|--------|--------|
| 建议 1（修正 total_notes） | P0 | ✅ 已完成 |
| 建议 2（文件同步机制） | P1 | P2，v0.6+ 长期规划 |
| 建议 3（定期一致性检查） | 近期执行 | ✅ 已覆盖（deploy.sh）|

### Tunnel 公网自检

| 任务 | 说明 | 状态 |
|------|------|:----:|
| **公网可达性检测** | tunnel.sh 每 60s 检查公网 URL | ✅ |
| **自动重启** | 连续 2 次不可达自动杀 lt 重连 | ✅ |
...
| 模块 | 规则（PRODUCT.md） | 代码实现 | 状态 |
|------|-------------------|---------|:----:|
| **基础采集** | 10星点 | base_star: 10 | ✅ 一致 |
| **每日倍数** | 1/1.5/2/0.2 | daily_multipliers: [1.0, 1.5, 2.0, 0.2] | ✅ 一致 |
| **连续加成** | 7天+10%/30天+30% | streak_bonuses 配置存在 | ✅ 一致 |
| **跨界加成** | ≥2领域标签额外+5 | cross_domain_bonus: 5 | ✅ 一致 |
| **连接力奖励** | ≥5+50/≥20+100/≥50+200 | link_power_rewards 配置存在 | ✅ 一致 |
| **周回顾奖励** | 200星点 | weekly_review_reward: 200 | ✅ 一致 |
| **月度战报奖励** | 500星点 | monthly_report_reward: 500 | ✅ 一致 |
| **消费汇率** | 1.0 | coupon_rate: 1.0 | ✅ 一致 |
| **基金汇率** | 1.0+0.5 | fund_base_rate: 1.0, fund_base_bonus: 0.5 | ✅ 一致 |
| **星点记录** | - | _log_star_income() 记录所有来源 | ✅ 存在 |

### 发现的问题

| 问题 | 说明 | 严重性 | 状态 |
|------|------|:------:|:----:|
| **API 未返回连接力数值** | 前端无法展示当前连接力值 | 🔴 | ✅ 已修复 |
| **API 未返回跨界笔记数** | cross_domain_notes_count 存在但未暴露 | 🟡 | ✅ 已修复 |
| **连接力公式未文档化** | 代码中有 `图密度×100×log₂(节点数+1)×(跨界占比+0.1)` | 🟢 | 📋 |
| **daily_multipliers 索引越界** | count > 4 时取最后一项而非报错 | 🟢 | 📋 |
| **star_income_history 上限100条** | 旧记录会被截断 | 🟢 | 📋 |

### 建议优化项

- [x] API 返回 `current_link_power` 字段 ✅ 已完成
- [x] API 返回 `cross_domain_notes_count` 字段 ✅ 已完成
- [ ] 前端能力面板展示连接力数值
- [ ] 考虑增加星点历史记录的展示功能
...
```
单条星点 = base_star × daily_multiplier[count-1]
        + (count == 1 ? 连续加成 : 0)
        + (跨界 ? cross_domain_bonus : 0)

连续加成 = streak_days ≥ 30 ? +30% : streak_days ≥ 7 ? +10% : 0
