...
- [ ] **兑换模块下线**：兑换体系过于复杂，暂不维护。v0.2.5 先通过 feature flag 关闭前端入口和路由暴露，代码不动；拆分/剥离并入后续 v0.3 结构调整
...
> 版本归属以 [PRODUCT.md](PRODUCT.md#三后续版本计划) 版本表为准，此处仅列出方向不做编号。

- [ ] **前端文件拆分**：index.html 超 2600 行，CSS → styles.css，JS → app.js
- [ ] **后端重构**：engine.py 按功能拆分为子模块
- [ ] **赛季主题系统**：开拓者/连线大师/深度矿工/分享者 四主题 + app.js 按模块拆分
- [ ] **自定义挑战**：用户自行设定挑战目标与奖励，自动追踪进度
- [ ] **能力值面板**：星点里程碑档位制 + 前端能力面板展示
- [ ] **Tab 导航**：前端 6 个 Tab(采集/概览/成长/标签/行动/规则)，Alpine.js x-show 切换
